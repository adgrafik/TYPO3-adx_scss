	...
	public function drawRTE(FormEngine $parentObject, $table, $field, $row, $PA, $specConf, $thisConfig, $RTEtypeVal, $RTErelPath, $thePidValue) {
		...
		if (!self::$coreLoaded) {
			...
			foreach ($contentCSS as $key => $pathAndFilename) {
				...
				$configuration = \AdGrafik\AdxScss\Utility\ScssUtility::getConfiguration($thePidValue);

				$cssFiles[] = GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $scss->compile($cssFile, $configuration) . '?' . filemtime($cssFile);
			}
