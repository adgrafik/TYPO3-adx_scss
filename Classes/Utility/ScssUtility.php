<?php
namespace AdGrafik\AdxScss\Utility;
...
class ScssUtility {
	...
	public static function getConfiguration($contentObject) {
		...
		if (self::$configuration === NULL) {
			...
			if (is_object($contentObject)) {
				...
				self::$configuration = isset(self::$configuration['plugin.']['tx_adxscss.'])
					? self::$configuration['plugin.']['tx_adxscss.']
					: array();
				...
				self::$configuration = isset($tsParser->setup['plugin.']['tx_adxscss.'])
					? $tsParser->setup['plugin.']['tx_adxscss.']
					: array();
			}
		}
	...
	public static function includeCss($content, array $configuration = array()) {
		...
		if (isset($configuration['file'])) {
			$compiledPathAndFilename = $scss->compile($configuration['file'], $compilerSettings);
		}

		self::addCssFile($compiledPathAndFilename, $includeCssSettings);

		if (isset($configuration['data'])) {
			$compiledPathAndFilename = $scss->compile($configuration['data'], $compilerSettings);
		}
